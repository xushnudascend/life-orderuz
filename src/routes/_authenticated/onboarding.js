...
import { bmiLabel, calcBMI, sectionQuestions, firstTaskFromAnswers, } from "@/lib/onboarding";
...
function Onboarding() {
    ...
    if (ahaNudge !== null) {
        return (<div className="min-h-dvh bg-background text-foreground relative overflow-hidden flex items-center justify-center">
        ...
        <div className="mx-auto max-w-2xl px-5 py-16 relative z-10 w-full">
          <div className="animate-fade-in-up rounded-[32px] border border-border/40 bg-secondary p-8 md:p-12 shadow-premium backdrop-blur-4xl">
            ...
            {archetypeName && (<p className="mb-6 font-ui text-[10px] font-bold uppercase tracking-[0.3em] text-primary/80">
                {t("brand.disciplineScore")} · {archetypeName.replace('_', ' ')}
              </p>)}
            {ahaNudge && (<div className="whitespace-pre-line rounded-2xl border border-primary/20 bg-primary/5 p-6 font-body text-base leading-relaxed text-foreground shadow-inner">
                {ahaNudge}
              </div>)}
            ...
            <Button className="mt-10 w-full h-14 rounded-full font-ui text-base font-semibold shadow-[0_20px_40px_-12px_hsl(var(--primary)/0.5)] transition-all hover:scale-[1.02]" size="lg" onClick={() => navigate({ to: "/dashboard" })}>
              {t("onboarding.start")}
      ...
      <div className="mx-auto max-w-xl px-5 py-10">
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {Array.from({ length: total }, (_, i) => {
            const done = i < bQuestions.length
                ? isAnswered(bQuestions[i])
                : aQuestions.every(isAnswered) && plan !== null;
            const active = i === step;
            const reachable = done || active || i <= step + 1;
            return (<button key={i} type="button" disabled={!reachable} onClick={() => reachable && setStep(i)} className={"h-7 min-w-7 rounded-md border px-2 font-ui text-[11px] tabular-nums transition-colors " +
                    (active
                        ...
                        : done
                            ...
                            : reachable
                                ...
                                : "border-dashed border-border/40 text-muted-foreground/40 cursor-not-allowed")} aria-label={`Qadam ${i + 1}`}>
                {i + 1}
              </button>);
        })}
        </div>
        ...
        <div className="mt-10 animate-fade-in-up">
          {currentB ? (<QuestionCard q={currentB} value={answers[currentB.key]} onChange={(v) => {
            ...
            }} onToggleMulti={(v) => toggleMulti(currentB.key, v)}/>) : (<FinalPage questions={aQuestions} answers={answers} onChange={(k, v) => setAnswer(k, v)} bmi={bmi} plan={plan} onPlanChange={setPlan} t={t}/>)}
        </div>

        <div className="mt-10 flex items-center justify-between">
          <Button variant="ghost" className="font-ui" onClick={goBack} disabled={step === 0 || saving}>
            <ArrowLeft className="mr-1.5 h-4 w-4"/>
            {t("onboarding.back")}
          </Button>

          <Button size="lg" className="group h-12 rounded-full px-7 font-ui font-semibold transition-all duration-300 hover:shadow-[0_12px_40px_-16px_hsl(var(--primary)/0.7)] active:scale-[0.98] disabled:opacity-40" onClick={goNext} disabled={!canAdvance || saving}>
            {saving && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
            {isFinalStep ? t("onboarding.finish") : t("onboarding.next")}
            {!isFinalStep && (<ArrowRight className="ml-1.5 h-4 w-4 transition-transform duration-300 group-hover:translate-x-0.5"/>)}
          </Button>
        </div>
      </div>
    </div>);
...
function QuestionCard({ q, value, onChange, onToggleMulti, }) {
    return (<div>
      <h2 className="font-serif text-3xl leading-tight tracking-tight text-balance">{q.prompt}</h2>
      {q.helper && (<p className="mt-3 font-ui text-sm leading-relaxed text-muted-foreground">{q.helper}</p>)}
      <div className="mt-8">
        {q.type === "number" ? (<div className="flex items-baseline gap-3">
            <Input type="number" inputMode="numeric" min={q.min} max={q.max} value={typeof value === "string" ? value : ""} onChange={(e) => onChange(e.target.value)} className="w-40 font-serif text-3xl h-auto py-3"/>

            {q.suffix && <span className="font-ui text-sm text-muted-foreground">{q.suffix}</span>}
          </div>) : (<OptionList q={q} value={value} onChange={onChange} onToggleMulti={onToggleMulti}/>)}
      </div>
    </div>);
}
function OptionList({ q, value, onChange, onToggleMulti, }) {
    ...
    return (<div className="space-y-2">
      {q.options?.map((opt) => {
            ...
            return (<button key={opt.value} type="button" onClick={() => (isMulti ? onToggleMulti(opt.value) : onChange(opt.value))} className={selected
            ...
            <span aria-hidden className={selected
                    ? isMulti
              ...
              {selected && (<span className={isMulti
...
function FinalPage({ questions, answers, onChange, bmi, plan, onPlanChange, }) {
    ...
    const [openKey, setOpenKey] = useState(firstUnanswered);
    function toggle(key) {
      ...
      <div className="divide-y divide-border/40 overflow-hidden rounded-[var(--radius-md)] border border-border/60 bg-card/30">
        {questions.map((q) => {
            ...
            return (<div key={q.key}>
              <button type="button" onClick={() => toggle(q.key)} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left transition-colors hover:bg-card/60" aria-expanded={isOpen}>
                <div className="min-w-0 flex-1">
                  <p className="font-ui text-sm font-medium text-foreground">{q.prompt}</p>
                  <p className={"mt-1 truncate font-ui text-xs " +
                    (answered ? "text-primary" : "text-muted-foreground")}>
                    {summary(q)}
                  </p>
                </div>
                <span aria-hidden className={"shrink-0 font-ui text-lg text-muted-foreground transition-transform " +
                    (isOpen ? "rotate-45" : "")}>
                  +
                </span>
              </button>
              {isOpen && (<div className="border-t border-border/40 px-5 pb-5 pt-4 animate-fade-in">
                  {q.type === "number" ? (<div className="flex items-baseline gap-3">
                      <Input type="number" inputMode="numeric" min={q.min} max={q.max} value={typeof answers[q.key] === "string" ? answers[q.key] : ""} onChange={(e) => onChange(q.key, e.target.value)} className="w-32 font-serif text-2xl h-auto py-2"/>

                      {q.suffix && (<span className="font-ui text-sm text-muted-foreground">{q.suffix}</span>)}
                      {q.key === "profile.weight_kg" && bmi && (<span className="font-ui text-xs text-muted-foreground ml-2">
                          BMI: <span className="text-foreground">{bmi}</span> ({bmiLabel(bmi)})
                        </span>)}
                    </div>) : (<OptionList q={q} value={answers[q.key]} onChange={(v) => {
                            onChange(q.key, v);
        ...
        {(() => {
            ...
            return (<div>
              <button type="button" onClick={() => toggle("__plan__")} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left transition-colors hover:bg-card/60" aria-expanded={isOpen}>
                <div className="min-w-0 flex-1">
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelled)
                    setData({ sameArchetype: res.sameArchetype, samePlan: res.samePlan });
            }
            catch {
                /* ignore */
            }
      ...
      <div className="relative rounded-[var(--radius-md)] border border-border/60 bg-background/60 p-5 backdrop-blur-sm">
        <div className="flex items-center gap-2 mb-3">
          <div className="flex -space-x-2">
            {[1, 2, 3].map(i => (<div key={i} className="h-6 w-6 rounded-full border-2 border-background bg-muted overflow-hidden">
...
function FirstTaskCard({ answers, t }) {
    const task = useMemo(() => firstTaskFromAnswers(answers), [answers]);
    return (<div className="mt-6 rounded-2xl border border-primary/20 bg-primary/5 p-6 shadow-[inset_0_0_20px_rgba(45,212,191,0.05)]">
