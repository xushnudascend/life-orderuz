CREATE TABLE IF NOT EXISTS public.weekly_reports (
  ...
  habit_completion_pct numeric NOT NULL DEFAULT 0,
...
DROP POLICY IF EXISTS "own weekly reports" ON public.weekly_reports;
CREATE POLICY "own weekly reports" ON public.weekly_reports
  FOR SELECT TO authenticated USING (user_id = auth.uid());

-- Haftalik statistikani server tomonda hisoblaydi (cron uchun).
...
BEGIN
  ...
  agg AS (
    SELECT
      ...
      COALESCE((SELECT max(st.current_streak) FROM public.streaks st WHERE st.user_id = a.user_id), 0)::int AS streak,
      GREATEST((SELECT count(*) FROM public.habits h WHERE h.user_id = a.user_id AND h.archived_at IS NULL), 1)::int AS habit_count
    FROM active a
  )
